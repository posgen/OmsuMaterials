    ...
    свою метку (label). Структура исходных данных в рамках логики pandas становится:
        ...
        { [label_row1] ||      4     | |     3.5     |     "AB"   |  2022-02-09  | } <- строка 
          ...
          \______________________________________________________________________/
                                            |
                                            v
                    объект типа DataFrame (pandas.DataFrame, с учётом имени модуля)
...
'''
    ...
    чем с объектам Series. Основные правила:
        ...
        -> (4) выбор среза строк                        |   df[idx_start:idx_end]  |         DataFrame
                                                        | df.loc[row_lbl1:row_lbl2]|         DataFrame
                                                        | df.iloc[idx1:idx2]       |         DataFrame
    ...
    *[True, False, False, True, False]*, в результате которой будут выбраны только те строки, которые соответствуют
    ...
    Формально, используется не только списко, а любой перечислимый объект python'а.


    Кроме того, везде, где возвращаемым объектом является объект DataFrame, мы получаем копию данных, которую
    можно менять независимо от исходного набора.
    
    При выборе строк из объекта DataFrame можно указать, какие столбцы нужно выбрать из исходного набора данных. Это
    касается всех способов по выбору строк, за исключением (5). Для этого методу досптупа передаётся второй аргумент. 
    В общем виде, получаются такие конструкции:
        df.loc[row_label, [col_lable1, col_lable2]] <- выбор конкретной строки, но только двух столбцов;
        df.iloc[2:5, col_label1:col_labelN] <- выбор только нужных строк и столбцов.


    Для примеров, создадим DataFrame из первого комментария.
'''
source_data = {
    'factor': [4, 6, 7, 8], 
    'rate': [3.5, 1.6, 8.4, 2.5],
    'what': ['AB', 'CD', 'WX', 'QN'],
    'when': [datetime(2022, 2, 9), datetime(2022, 2, 13), datetime(2022, 2, 13), datetime(2022, 2, 26)]
}

target_df = pd.DataFrame(source_data, index=['e1', 'e2', 'e3', 'e4'])
print(f"[Шаг 14] исходный набор данных:\n{target_df}")

'''
    Способы (1) - (4) - достаточно тривиальны.

    Пример с логическим списком. Здесь важно, чтобы размер переданного списка логических значений совпадал с числом строк.
    Далее будут выбраны только первая и последня строки.
'''
print(f"[Шаг 14] выбор строк по булевым значениям:\n{target_df[[True, False, False, True]]}")
print(f"[Шаг 14] выбор строк и столбцов одновременно через срезы:\n{target_df.loc['e2':, 'rate':'what']}\n")

'''
    Для объекта DataFrame столбцы могут добавляться динамически. Это делается либо с помозью оператора индексации (1), либо
    с помощью метода assign. Различия в том, что первый способ меняет исходный набор данных, а второй - возвращает новую
    копию набора данных + добавленные столбцы.

        df["New column"] = df["A"] * df["B"]
        df["Another new column"] = [3, 4, 5, 6, 7, 8]

        new_df = df.assign(NewRation=[3.4, -1.2, -3.4, 5.6])
        new_df = df.assign(NewCol=lambda f: f['A'] / f['B'])
    В случае метода assign название аргумента метода (*NewRatio*, *NewCol* в примере) становится названием нового столбца.
    При передаче замыкания (*lambda* выше) его аргументом является исходный объект DataFrame, то есть *f* это тоже самое, что 
    и *df* в примере.
'''
target_df["ratio"] = target_df["rate"] / target_df["factor"]
print(f"[Шаг 15] добавили столбец:\n{target_df}")
print(f"[Шаг 15] пример с assign:\n{target_df.assign(X=lambda f: f['ratio'] ** 2 )}")
print(f"[Шаг 15] но target_df не поменялся:\n{target_df}\n")

'''
    И основной инструмент анализа данных - их фильтрация по различным условиям. Для этих целей как раз и существует способ (5)
    выбора части данных из исходного набора. А также векторизованные логические операции. На практике, общая схема выбора данных
    по условиям выглядит следующим образом:
        1. Находим строки удовлетворяющие определённому условию -> получаем массив булевых значений.
        2. Если условий больше одного - объединяем их по логике выбора (всякие логичесие "И", логические "ИЛИ").
           В результате объединения на выходе - всё равно массив булевых значений.
        3. Применяем выборку данных через способ (5).\

    Для примера, есть у нас некий DataFrame - df - с тремя столбцами "x", "y", "z", представляющим трёхмерные координаты
    в диапазоне от [-5; 5]:
'''
source_data = {
    'x': 10 * np.random.random_sample(10) - 5,
    'y': 10 * np.random.random_sample(10) - 5,
    'z': 10 * np.random.random_sample(10) - 5,
}

index_values = [f"point_{i}" for i in range(1, 11)]
target_df = pd.DataFrame(source_data, index=index_values)
print(f"[Шаг 16] исходный набор:\n{target_df}")

# Выбирать все строки, где координата x > 2, синтаксически делается так:
part_df = target_df[target_df['x'] > 2.0]
print(f"[Шаг 16] все точки с x > 2.0:\n{part_df}")
'''
    Как раз используется то, что можем выбирать строки по массиву логических значений, который возвращается выражением:
        target_df['x'] > 2.0

    Объединяются логические условия с помощью операторов: 
        -> "&" - логическое "И";
        -> "|" - логическое "ИЛИ".

    Так, выбрать все точки, координаты x и  y которых попадают в диапазоны:
        x: [0; 2]
        y: [-2; 2]
'''
part_df = target_df[(target_df['x'] > 0) & (target_df['x'] < 2) 
                    & (target_df['y'] > -2) & (target_df['y'] < 2)]
print(f"[Шаг 16] все точки внутри квадрата на плоскости XY (1):\n{part_df}")
'''
    ! круглые скобки вокруг каждого из индивидуальных условий выбора - обязательны из-за приоритета операторов языка !

    Поскольку данные создаются случайным образом, при разных попытках запуска подобного скрипта будут разные результаты.
    В том числе - и пустой набор данных.

    Кому эстетически непривлекательно смотреть на кучи скобок, pandas предлагает метод query, позволящий задавать условия
    в виде строки:
'''
part_df = target_df.query("(x > 0) & (x < 2) & (y > -2) & (y < 2)")
print(f"[Шаг 16] все точки внутри квадрата на плоскости XY (2):\n{part_df}")

part_df = target_df.query("x < y < z")
print(f"[Шаг 16] более хитрая комбинация:\n{part_df}\n")
'''
    Аналогичным образом можно создавать сколь угодно сложные фильтры.

    Для различных текстовых данных pandas поддерживает широкий набор методов для проверки/сопоставления строк. Смотреть, 
    к примеру, тут - https://pandas.pydata.org/docs/user_guide/text.html#testing-for-strings-that-match-or-contain-a-pattern.
'''

# так, для завершения
target_df.at['point_2', 'y'] = 15
target_df.iat[4, 2] = -88
print(f"[Шаг Заключительный] после изменения:\n{target_df}\n")