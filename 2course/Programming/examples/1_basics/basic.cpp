...
/*
    ...
    Файлы с расширением ".h" называются заголовочными, и, как правило, включают в себя объявления всех предоставляемых функций, констант и структур данных. Однако, для стандартной библиотеки убрали ".h" из названия заголовочных файлов
    В данном случае подключаются три библиотеки:
        iostream     - предоставляет функции ввода / вывода как на экран;
        
        сstdlib    - набор полезных функций из языка С, включающий в себя функции преобразования строки в число, создание случайного числа и некоторые другие;
...
/*
    ...
    Одновременно с объявлением переменной, можно присвоить ей начальное значение.
    В С++ есть три способа присвоения: 
        ...
        3) int var3{5};  // Синтаксис допустим начиная со стандарта С++ 2011 года (C++11). Как правило, самые распостраннёные компиляторы начиная с 2013/2014 года имеют полную поддержку поддержку этого стандарта.

    Также переменная может быть объявлена неизменяемой с помощью ключевого слова const.
    Например: const int MAX = 10;
...
double calculate_discriminant(double a, double b, double c)
{
    ...
    // Возращение результата вычисления из функции. Тип переменной result совпадает с типом возращаемого значения.
    return result;
}
...
/*
    ...
    Эти сведения преведены в самом начале из-за того, что вся стандартная библиотека С++ помещена в пространоство имён std. А также и
...
void print_data_size()
{
    ...
    */
    ...
    // Обязательным не является и применяется в случаях сложного ветвления логики выполнения. Потому и закомментирована следующая строка. 
    // return;
}
