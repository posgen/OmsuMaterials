/*
    ...
    После объявления переменных для работы с указателем ему обязательно нужно присвоить значение:
        ...
        // printf("%lf", *p_dbl); // пытаемся напечатать значение типа double из области памяти,
...
int main(void)
{
    ...
    /*
        ...
        ячеек в памяти.
    */
    printf("The address of variable i_num is %p\n", p_int);
    printf("The address of variable f_num is %p\n", p_dbl);

    // Демонстрация разыменования указателей
    *p_int = 28; // изменяем значение по адресу, который содержит указатель p_int
    printf("p_int points to value %d, i_num has value: %d\n", *p_int, i_num); // два напечатанных числа будут совпадать

    *p_dbl = 12.5 + *p_int; // с помощью разыменования, можно использовать указатели и в арифметических выражениях
    printf("p_dbl points to %.2lf\n", *p_dbl); // напечатает на консоли строку "p_dbl points to 40.50".
                                             // Переменная f_num теперь равна этому же (40.5) числу.

    // Обратная связь тоже работает
    f_num = 5.8;
    printf("p_dbl points to %.2lf\n", *p_dbl); // печатаем "p_dbl points to 5.80"
