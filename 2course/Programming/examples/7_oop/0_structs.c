/*
    ...
    элементов под одним названием. Или, более неформально: это объединение кучи переменных
    под единными.
    ...
    <тип_1> и <переменная_1> - обычное определение переменной любого доступного типа данных языка.
...
int main(void)
{
    ...
    (*p_point).x = 3;
    // Но лучше воспользоваться специальным олператором ->
