/*
    ...
    Весь код из 0_structs.c - рабочий и в C++. Разве что, трюк с typedef в языке C++ не нужен. Если мы определили
    структурный тип, то его переменные можно объявлять без ключевого слова struct:
        struct SomeData
        {
            int num;
            char name[50];
    ...
    в общем-то, не интересно для потребителя. Взаимодействуем с ней только через строго определённые функции
    (интерфейс).
    ...
    Эта идея и состовляет основную концепцию ООП.
    ...
    И не зря! Добавление полей класса делается абсолютно аналогично добавлению полей структуры. Например,
        ...
        Coord crd1, crd2; // crd1, crd2 - объекты класса Coord. И да, выше указанного кода 
                          // достаточно для объявлениря переменных.
    ...
    Такие поля называют закрытыми (private). Формально, полное определение указанного выше класса такое:
        ...
        private:         // метка private ставится по умолчанию, так что можно не указывать её.
            int x, y, z; // данные поля и сохраняют состояние конкретного объекта
        };
    ...
    Блоки с private и public полями могут произвольно чередоваться, например:
        ...
        private:              // начиная со следующей строки, все объявления опять являются закрытыми
            int y, z;
        };
    ...
    они иметь какую-то особенность, не зря их внутрь класса запихивали?
    ...
    значения из конкретного объекта. Чтобы не терять мысль, к примеру:
        ...
        public:
            ...
            void set_coord(int _x, int _y, int _z); // определим эту функцию вне класса
        };
    ...
    Объединение полей и методов в классе (типе данных), как например в Coord выше, называется 
    в ООП - **инкапсуляцией**.
...
/*
    Данный файл - только начало набора программ по ООП, в котором на примере
